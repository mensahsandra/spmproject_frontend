...
const LecturerAssessmentPageContent: React.FC = () => {
  ...
  const handleCreateAssessment = async () => {
    ...
    // Prepare questions based on format
    let questions: any[] = [];
    if (assessmentFormat === 'Multiple Choice') {
      questions = multipleChoiceQuestions;
    } else if (assessmentFormat === 'Description/Typing') {
  ...
  // Descriptive Questions Management
  const addDescriptiveQuestion = () => {
    setDescriptiveQuestions(prev => [...prev, '']);
  };

  const updateDescriptiveQuestion = (index: number, value: string) => {
    setDescriptiveQuestions(prev => prev.map((q, i) => i === index ? value : q));
  };

  const removeDescriptiveQuestion = (index: number) => {
    if (descriptiveQuestions.length > 1) {
      setDescriptiveQuestions(prev => prev.filter((_, i) => i !== index));
    }
  };
                ...
                {!selectedCourse ? (
                  ...
                  <>
                    ...
                    {assessmentFormat === 'Description/Typing' && (
                      <div className="row mb-3">
                        <div className="col-12">
                          <label className="form-label fw-bold">Questions</label>
                          <div className="d-flex justify-content-between align-items-center mb-3">
                            <span>Questions: {descriptiveQuestions.length}</span>
                            <button 
                              type="button"
                            ...
                            <div key={index} className="card mb-2">
                              <div className="card-body">
                                <div className="d-flex justify-content-between align-items-start mb-2">
                                  ...
                                  {descriptiveQuestions.length > 1 && (
                                    <button 
                                      type="button"
                                      className="btn btn-outline-danger btn-sm"
                                      onClick={() => removeDescriptiveQuestion(index)}
                                    >
                                      Remove
                                    </button>
                                  )}
                                ...
                                <textarea 
                                  ...
                                  value={question}
                                  onChange={(e) => updateDescriptiveQuestion(index, e.target.value)}
                                />
                              </div>
                            </div>
                          ))}
